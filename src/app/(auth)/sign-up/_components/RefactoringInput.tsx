...
type InputProps = {
    ...
    icon?: ReactNode;
    label?: string;
    description?: string; // ✅ 항상 보여지는 메시지
    validationMessage?: string | ReactNode; // ✅ 조건부 메시지
    handleClickIcon?: () => void;
    rules?: RegisterOptions; // ✅ react-hook-form validation
} & InputVariantProps &
    ComponentProps<'input'>;

const RefactoringInput = ({
    name,
    state,
    icon,
    iconPosition = 'right',
    ...
    return (
        <Controller
            name={name}
            control={control}
            rules={rules} // ✅ 외부에서 받은 validation 규칙 전달
            render={({ field }) => (
                <div className="w-full flex flex-col gap-3 mo:gap-2">
                    ...
                    {(validationMessage || errorMessage) && (
                        <span className={textVariant({ state: 'error' })}>{validationMessage || errorMessage}</span>
                    )}
                </div>
            )}
        />
...
export default RefactoringInput;
